...
const MyProjects = () => {
  ...
  const fetchProjects = async () => {
    ...
    try {
      const confirm =  window.confirm('Are you sure you wanna delete the project?');
      if(!confirm) return;
            ...
            <div className="flex flex-wrap gap-3.5">
              {projects.map((project) => (
                <div onClick={()=>navigate(`/projects/${project.id}`)}
                  key={project.id}
                  className=" relative group w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden shadow-md group hover:shadow-indigo-700/30 hover:border-indigo-800/80 transition-allduration-300 "
                >
