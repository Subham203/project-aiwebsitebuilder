  ...
  return (
    ...
    >
      <div className="flex flex-col h-full">
        {/* Message container */}
        <div className="overflow-y-auto no-scrollbar px-3 flex flex-col gap-4">
              ...
              if (isMessage) {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-5 leading-relaxed ${isUser ? "bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-none" : "bg-gray-800 rounded-tl-none text-gray-100"}`}
                    >
                      {msg.content}
                    </div>
                    {isUser && (
                ...
                return <div key={ver.id} className="w-4/5 mx-auto my-2 p-3 rounded-xl bg-gray-800 text-gray-100 shadow flex flex-col gap-2">
                  ...
                  <div className="flex items-center justify-between">
                    {project.current_version_index===ver.id ?(
                      ...
                      <button onClick={()=>handleRollback(ver.id)} className="px-3 py-1 rounded-md text-xs bg-indigo-600 text-white">Roll back to this version</button>
                    )}
                    <Link target="_blank" to={`/preview/${project.id}/${ver.id}`}>
                      <EyeIcon className="size-6 p-1 bg-gray-700 hover:bg-indigo-500 transtition-colors rounded"/>
                    </Link>
                  </div>
                </div>;
              }
